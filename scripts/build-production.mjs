...
// Pages that ship their own self-contained inline <style> and intentionally
// do not load the Tailwind bundle.
const SELF_STYLED_PAGES = new Set(['drop.html', 'admin-login.html', 'admin-drop.html']);
...
// v2 cache prefix: every cache created by an older worker generation is
// invalidated wholesale on activation, so a previously cached shell can never
// be served again.
const CACHE_NAME = 'valmont-v2-${cacheVersion}';
...
self.addEventListener('activate', event => {
  event.waitUntil((async () => {
    ...
    // Hard reset: anything that is not this build's cache is deleted, which
    // covers caches left behind by older (including broken) deployments.
    await Promise.all(stale.map(key => caches.delete(key)));
    // If we evicted something we are taking over from a previous generation.
    // Cached HTML is the only entry that can pin an unstyled page, so drop it
    // and re-fetch it immediately to keep the offline shell intact.
    if (stale.length) {
      const cache = await caches.open(CACHE_NAME);
      const entries = await cache.keys();
      await Promise.all(entries.filter(isDocument).map(request => cache.delete(request)));
      await Promise.allSettled(
        [...PUBLIC_NAVIGATIONS].map(url => cache.add(new Request(url, { cache: 'reload' }))),
      );
    }
...
for (const page of pages) {
  ...
  // Regression guard. PR #68 rewrote index.html's <head> and silently dropped
  // its stylesheet <link>s; because scripts/build-landing-pages.js derives the
  // landing-page <head> from index.html, that one edit unstyled all 35 pages
  // while every other check still passed. A styled page with no Tailwind link
  // is a raw-HTML page, so fail the build instead of publishing it.
  if (!SELF_STYLED_PAGES.has(page) && !html.includes(`href="${manifest['tailwind.min.css']}"`)) {
    throw new Error(
      `${page}: missing fingerprinted Tailwind stylesheet (${manifest['tailwind.min.css']})`,
    );
  }
}
