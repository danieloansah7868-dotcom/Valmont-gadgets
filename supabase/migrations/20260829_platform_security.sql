-- ==============================================================================
-- Valmont Gadgets — Platform security model: swap & sell, UK/US used, wholesale,
--                       store partners, promotions, audit
--
-- Migration: 20260829_platform_security.sql
--
-- WHY A FOLLOW-UP FILE AND NOT AN EDIT OF 20260828_platform_tables.sql
-- --------------------------------------------------------------------
-- That migration was already applied to the live project, and this repository's
-- rule (docs/PRODUCTION_RUNBOOK.md §3) is that an applied migration is never
-- rewritten in place. What it applied is unsafe to keep as-is, so this file
-- converges it and owns the security model from here on:
--
--   * the tables it created had no GRANT statements at all, so they inherited
--     Supabase's default browser grants, while their policies were written
--     without a TO clause (so they applied to every role, anon included);
--   * its admin policies read public.admin_allowlist directly, which
--     20260811_admin_email_allowlist.sql revoked from browser roles — kept as-is
--     that turns ordinary reads of these tables into a runtime error;
--   * it stored seller passwords in `sellers.password_hash` and Ghana Card
--     numbers in the clear (`ghana_card`, `face_photo_url`);
--   * `ad_payments.status DEFAULT 'completed'` meant a row written from the
--     browser was a *paid* promotion by default.
--   * no RPC existed, so the platform pages could not read or write anything.
--
-- Design rules encoded below (mirroring 20260814000100_production_hardening.sql
-- and 20260811_admin_email_allowlist.sql):
--
...
--     what it stores, and logs itself. Nothing here can be faked from devtools:
--     no client-supplied price, seller identity, approval, ban lift, or "paid".
--   * Reads the public needs are narrow functions. No function returns a Ghana
--     Card in the clear, a supplier cost, or unpublished stock to someone who is
--     neither the owner nor the allowlisted admin.
--   * Anti-spam budgets are enforced per account in the database. The counter in
--     assets/js/security.js only paces the UI.
--   * Safe to re-run: IF NOT EXISTS / DROP POLICY IF EXISTS / ON CONFLICT, and a
--     fail-closed privilege post-condition at the end.
--
...
-- ── 0. Converge whatever 20260828 left behind ────────────────────────────────
-- 0a. Its policies must not survive: they carry no TO clause, and their admin
--     predicate reads a table browser roles can no longer see.
DO $$
DECLARE
    entry record;
BEGIN
    FOR entry IN
        SELECT * FROM (VALUES
            ('sellers',          'Sellers can manage own data'),
            ('sellers',          'Admin full access on sellers'),
            ('swap_listings',    'Public can read active swap listings'),
            ('swap_listings',    'Sellers can manage own listings'),
            ('swap_listings',    'Admin full access on swap_listings'),
            ('swap_leads',       'Admin full access on swap_leads'),
            ('used_inventory',   'Public can read available used inventory'),
            ('used_inventory',   'Admin full access on used_inventory'),
            ('wholesale_dealers','Admin full access on wholesale_dealers'),
            ('wholesale_orders', 'Admin full access on wholesale_orders'),
            ('partner_applications', 'Admin full access on partner_applications'),
            ('ad_payments',      'Admin full access on ad_payments'),
            ('admin_audit_log',  'Admin full access on admin_audit_log')
        ) AS legacy(tablename, policyname)
    LOOP
        IF to_regclass('public.' || entry.tablename) IS NOT NULL THEN
            EXECUTE format('DROP POLICY IF EXISTS %I ON public.%I', entry.policyname, entry.tablename);
        END IF;
    END LOOP;
    -- Indexes that pointed at columns this file removes.
    DROP INDEX IF EXISTS public.idx_swap_listings_seller;
    DROP INDEX IF EXISTS public.idx_sellers_ghana_card;
    DROP INDEX IF EXISTS public.idx_ad_payments_seller;
END;
$$;

-- 0b. Columns this file needs that are safe to add in place. Operational rows
--     (a used board someone has already typed up, a dealer order already taken
--     by hand) are kept; only tables whose SHAPE is security-relevant are
--     rebuilt below.
ALTER TABLE IF EXISTS public.used_inventory ADD COLUMN IF NOT EXISTS sold_at timestamptz;
ALTER TABLE IF EXISTS public.used_inventory ADD COLUMN IF NOT EXISTS updated_at timestamptz
    NOT NULL DEFAULT timezone('utc', now());
ALTER TABLE IF EXISTS public.wholesale_orders ADD COLUMN IF NOT EXISTS unit_count integer NOT NULL DEFAULT 0;
ALTER TABLE IF EXISTS public.wholesale_orders ADD COLUMN IF NOT EXISTS subtotal numeric(12,2) NOT NULL DEFAULT 0;
ALTER TABLE IF EXISTS public.wholesale_orders ADD COLUMN IF NOT EXISTS delivery_fee numeric(12,2) NOT NULL DEFAULT 0;
ALTER TABLE IF EXISTS public.wholesale_orders ADD COLUMN IF NOT EXISTS updated_at timestamptz
    NOT NULL DEFAULT timezone('utc', now());
ALTER TABLE IF EXISTS public.admin_audit_log ADD COLUMN IF NOT EXISTS details jsonb NOT NULL DEFAULT '{}'::jsonb;
-- A promotion that has not been verified is 'pending' — never the legacy default.
ALTER TABLE IF EXISTS public.ad_payments ALTER COLUMN status SET DEFAULT 'pending';

-- 0c. Rebuild any table whose columns do not match the shape below — but only
--     while it is empty. If somebody has already put real listings, orders or
--     applications in these tables, the migration stops and says so rather than
...
BEGIN
    FOR spec IN
        SELECT * FROM (VALUES
            ('sellers',
                ARRAY['auth_user_id','display_name','ghana_card_hash','ghana_card_masked','face_verified','is_banned'],
                ARRAY['password_hash','ghana_card','face_photo_url','role']),
            ('swap_listings',
                ARRAY['seller_auth_id','listing_type','images','status','is_promoted','views','leads_count'],
                ARRAY['seller_id']),
            ('swap_leads', ARRAY['seller_auth_id','buyer_auth_id','buyer_name','message','status'], ARRAY[]::text[]),
            ('wholesale_dealers',
                ARRAY['auth_user_id','business_name','contact_name','status','reviewed_by','reviewed_at'],
                ARRAY['seller_id','ghana_card']),
            ('partner_applications',
                ARRAY['auth_user_id','shop_name','ghana_card_hash','ghana_card_masked','plan','status'],
                ARRAY['ghana_card']),
            ('ad_payments',
                ARRAY['listing_id','seller_auth_id','plan_hours','amount','status','approved_at'],
                ARRAY['seller_id']),
            ('write_quota', ARRAY['actor_auth_id','action','window_start','used_count'], ARRAY[]::text[])
        ) AS t(tbl, required, forbidden)
    LOOP
        IF to_regclass('public.' || spec.tbl) IS NULL THEN
            CONTINUE;  -- fresh project: the CREATE TABLE statements below own it
        END IF;

        SELECT array_agg(c) INTO missing
        FROM unnest(spec.required) c
        WHERE NOT EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_schema = 'public' AND table_name = spec.tbl AND column_name = c
        );
        SELECT array_agg(c) INTO leftover
        FROM unnest(spec.forbidden) c
        WHERE EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_schema = 'public' AND table_name = spec.tbl AND column_name = c
        );
        IF missing IS NULL AND leftover IS NULL THEN
            CONTINUE;  -- already converged
        END IF;

        EXECUTE format('SELECT count(*)::bigint FROM public.%I', spec.tbl) INTO n;
        IF n > 0 THEN
            RAISE EXCEPTION
              'public.% has % row(s) and does not match the reviewed shape (missing: %; must go: %). Export the rows, empty the table, and re-run this migration — it will not delete marketplace data implicitly.',
              spec.tbl, n, coalesce(array_to_string(missing, ', '), 'none'),
              coalesce(array_to_string(leftover, ', '), 'none')
              USING ERRCODE = 'P0001';
        END IF;
        EXECUTE format('DROP TABLE public.%I CASCADE', spec.tbl);
        RAISE NOTICE 'public.% was empty and had the pre-security shape: rebuilding it', spec.tbl;
    END LOOP;
END;
$$;
