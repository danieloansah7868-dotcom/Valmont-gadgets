...
async function applyAdminSiteSettings() {
  ...
  if (infoBannerSpans[0] && settings.whatsapp) infoBannerSpans[0].innerHTML = `Call / WhatsApp to Order: <a href="https://wa.me/${settings.whatsapp.replace(/[^0-9]/g,'')}" target="_blank" class="text-[#ff8c00] hover:underline">${settings.whatsapp}</a>`;
}
...
function renderStorefrontGrid() {
  ...
  if (pageCount > 1) {
    // Matches app.js: active page in brand orange, inactive white with hover.
    productGrid.insertAdjacentHTML("afterend", `<nav class="product-pagination flex justify-center gap-2 py-6" aria-label="Product pages">${Array.from({length: pageCount}, (_, i) => {
      ...
      // Weight lives in the branch: Tailwind emits .font-bold after .font-black,
      // so a shared base `font-bold` would override the active `font-black`.
...
async function renderApprovedReviewsList(productId){
  ...
  container.innerHTML = approved.map(r => {
    const starHTML = Array(5).fill("").map((_, i) => `<svg class="w-3 h-3 ${i < r.rating ? 'text-[#ff8c00] fill-current' : 'text-gray-200'}" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>`).join("");
...
function setReviewFormStars(stars){
  ...
  starButtons.forEach((btn, idx) => {
    ...
    if (!starSvg) return;
    if (idx < stars) { starSvg.classList.add("text-[#ff8c00]", "fill-current"); starSvg.classList.remove("text-gray-300"); }
    else { starSvg.classList.remove("text-[#ff8c00]", "fill-current"); starSvg.classList.add("text-gray-300"); }
  });
}
